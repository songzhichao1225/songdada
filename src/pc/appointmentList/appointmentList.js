        ...
        <div>
          {
            this.state.list.map((item, i) => (
              <Row key={i}>
                ...
                <Col xs={{ span: 2 }}  style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}><div>{item.PublicStatus}<div onClick={this.Complaints} data-id={item.uuid} style={item.iscomplain === 1 ? { color: '#F6410C', fontSize: '12px' } : { display: 'none' }}>(有投诉)</div></div></Col>
