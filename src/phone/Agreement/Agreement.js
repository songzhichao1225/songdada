...
class Agreement extends React.Component {
  ...
  componentDidMount(){
    if(this.props.location.query===undefined){
      ...
      this.setState({flag:this.props.location.query.flag})
    }
    ...
    return (
      <div className="Agreement" >
        ...
        <div className="boss">
          ...
          <span className="block">4、截止报名时，活动匹配成功。<span className="weight">不管用户是否到场</span>，场馆方正常提供场地前提下（无用户投诉场馆未预留场地），平台方都须第一时间将扣除服务费后的场地费或按照充值会员协议确定的场地费支付给场馆方；<span className="weight">平台服务费=场地费*8%(平台充值会员的场馆，服务费由平台给用户的售卖价与场馆给平台的会员价所产生的差价确定)。</span></span>
          <span className="block">5、当有用户投诉场馆未预留场地后，平台方须在24小时内安排推广员进行核实，根据最终核实情况，决定是否将场地费支付给场馆方；</span>
          <span className="block">6、当场馆方申请提现时，平台须确保提现的场地费24小时内到账。否则平台每天须支付1‰提现金额的滞纳金。</span>
          ...
          <span className="block">5、<span className="weight">场馆方须支付场地费的8%作为平台服务费(平台充值会员的场馆，服务费由平台给用户的售卖价与场馆给平台的会员价所产生的差价确定)</span>，如场馆方给平台的价格为20元/小时，场馆方收到的场地费为18.4元/小时；</span>
          <span className="block">6、场馆方在特殊情况下须临时关闭场馆时，须第一时间在场馆端进行操作。</span>
          <span className="block">7、对于匹配中、匹配成功的找运动伙伴订单；仅预订场馆的订单。甲方须预留场地，否则甲方须支付赔偿金给乙方，赔偿标准：到场用户20元/人；未到场用户10元/人。乙方再支付赔偿金给该订单对应的所有用户。甲方未预留场地超过3次，乙方有权取消合作，双方结清账款。对于签订会员合同的场馆，按照合同里甲方违约认同。</span>
