...
import { PullToRefresh, DatePicker, Toast, Card, Modal, InputItem, List } from 'antd-mobile';
...
class orderPh extends React.Component {
  ...
  async getVenueReservationss(data) {
    ...
    if (res.data.code === 2000) {
      ...
      this.setState({ lookList: res.data.data, otherType: res.data.other.sporttype, macNum: res.data.data[0].c, value: 'l' })
  ...
  submitVal = () => {
    this.getReservationActivitieslist({
      page: 1, sport: this.state.sportIdVal, status: this.state.statusIdVal, publicuid: '', startdate: this.state.start === '选择开始日期' ? '' : this.state.start.toLocaleDateString(),
      enddate: this.state.end === '选择结束日期' ? '' : this.state.end.toLocaleDateString()
    })
    this.setState({
      Drawervisible: false
    })
  ...
  async VenueClickCancelPlace(data) {
    ...
    if (res.data.code === 2000) {
      ...
      this.setState({ modalTwo: false, lotime: [] })
    } else {
      Toast.fail('操作失败', 1)
    }
  }
  ...
  async VenueRemarksLabel(data) {
    ...
    if (res.data.code === 2000) {
      ...
      alert('预定情况详情', ko.placeName !== undefined && ko.placePhone !== undefined && ko.placeHui !== undefined && ko.placeQi !== undefined ? arrObj : '没有预定人信息...', [
  ...
  lookPlate = e => {
    ...
    if (e.currentTarget.dataset.type !== '3' && e.currentTarget.dataset.type !== '2') {
      if (e.currentTarget.dataset.type === '1') {
        ...
        this.VenueClickCancelPlace({ uuid: uuid, date: this.state.qiDate, venueid: num, other: '', time: time, sportid: this.state.liNum, type: 2 })
      }
                  ...
                  this.state.activeSon.map((item, i) => (
                    <Card key={i}>
                      ...
                      <Card.Body style={{ fontSize: '12px' }}>
                        <div><span style={{ display: 'block', width: '90px', float: 'left' }}>金额:{item.SiteMoney}</span><span style={{ marginLeft: '5%' }}>支付状态:{item.SiteMoneyStatus}</span></div>
                        <div style={item.reserve === 1 ? { display: 'none' } : {}}><span style={{ display: 'block', width: '90px', float: 'left' }}>应到人数:{item.Shouldarrive}人</span><span style={{ marginLeft: '5%' }}>报名人数:{item.TrueTo}人</span></div>
