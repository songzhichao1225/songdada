                ...
                <div key={i}>
                  <Card>
                    <Card.Header
                      ...
                      extra={<div className="cardHeader">{item.StartTime.slice(0, 10)}<br />{item.StartTime.slice(10, item.StartTime.length)} -{item.FinishedTime.slice(10, item.FinishedTime.length)}</div>}
                    />
                    <Card.Body className="cardBody">
                      <div><span className="footerOne" >金额:￥{item.SiteMoney}</span><span style={{ marginLeft: '5%' }}>支付状态:{item.SiteMoneyStatus}</span><span style={item.reserve === 1 ? { display: 'none' } : { display: 'block', width: '90px', float: 'left' }}>应到人数:{item.Shouldarrive}人</span></div>
                      <div><span className="footerOne" >时长:{item.PlayTime}小时</span><span style={item.reserve === 1 ? { display: 'none' } : {}}>报名人数:{item.TrueTo}人</span><i onClick={this.showModal} data-venueid={item.venueid} data-uid={item.uuid} className={item.PublicStatus === '匹配中' ? 'sendingTwo' : 'circumstanceT' && item.PublicStatus === '待出发' ? 'sendingTwo' : 'circumstanceT' && item.PublicStatus === '活动中' ? 'sendingTwo' : 'circumstanceT'} >
